...
!------------------ barrier -------------------
	...
	call Read_model_data( filename, ppohBEM_number_element_dof, &
				...
				ppohBEM_int_para_fc, ppohBEM_dble_para_fc, 10, &
				irank, comm, ierr )
	call MPI_Barrier( comm, ierr )
!------------------ barrier -------------------
	if(ierr .ne. 0) then
...
!  if(irank==0) print *, 'H-matrix time = ', st21-st3
	if(ldense) then
		...
		call MPI_Comm_create(MPI_COMM_WORLD, act_grp, act_comm, ierr)
 		if(irank .lt. act_nrank   .and.   ppohBEM_linear_solver == "BICGSTAB" ) then
...
!------------------ barrier -------------------
	...
	! removed the result printing for numerous bicgstab parallel jobs
	! call Print_result(filename, ppohBEM_nond, ppohBEM_nofc, &
	! 			ppohBEM_nond_on_face, st_ppohBEM_np, ppohBEM_face2node, &
	! 			ndim, ppohBEM_sol, irank, 11, ierr )
!	if ( irank .eq. 0 ) then
		print *, 'before HACApK time = ', sngl(st2 - st1), ' [sec]'
		print *, '       HACApK time = ', sngl(st21 - st3), ' [sec]'
		print *, '_____________all time = ', sngl(st4 - st1), ' [sec]'
!	endif
...
!***Initialization_MPI
	subroutine Initialization_MPI( comm, nrank, irank, ierr )
		...
		call MPI_Init(ierr)
		if(ierr .ne. 0)    print *, 'Error: MPI_Init failed !!!'
		call MPI_Comm_size(comm, nrank, ierr)
		if(ierr .ne. 0)    print *, 'Error: MPI_Comm_size failed !!!'
		call MPI_Comm_rank(comm, irank, ierr)
		if(ierr .ne. 0)    print *, 'Error: MPI_Comm_rank failed !!!'
		if(irank .eq. 0)   print *, 'Number of processes ', nrank
		call MPI_Barrier(comm, ierr)
		if(ierr .ne. 0)    print *, 'Error: MPI_Barrier #1 !!!'
...
!***Read_model_data
	subroutine Read_model_data( filename, ppohBEM_number_element_dof, &
		...
		type (coordinate), dimension(:),    allocatable, intent(out) :: st_ppohBEM_np
		integer,           dimension(:, :), allocatable, intent(out) :: ppohBEM_face2node
		integer,           dimension(:, :), allocatable, intent(out) :: ppohBEM_int_para_fc
		real*8,            dimension(:, :), allocatable, intent(out) :: ppohBEM_dble_para_fc
...
!!!!    input data file : ppohBEM_ndble_para_fc  !!!!
			...
			dims = (/ ppohBEM_nond, ppohBEM_nofc, ppohBEM_nond_on_face, ppohBEM_nint_para_fc, ppohBEM_ndble_para_fc /)

			call MPI_Bcast( dims(1), 5, MPI_INTEGER, 0, comm, ierr )
			if( ierr .ne. 0 )     print *, 'Error: MPI_Bcast#11'

			call MPI_Bcast( st_ppohBEM_np(1)%x, ppohBEM_nond*3, MPI_DOUBLE_PRECISION, 0, comm, ierr )
			if( ierr .ne. 0 )     print *, 'Error: MPI_Bcast#12'

			allocate( ppohBEM_face2node(ppohBEM_nond_on_face, ppohBEM_nofc), stat = ierr )
			if( ierr .ne. 0 )     print *, 'Memory allocation #11 failed'
			...
			call MPI_Bcast ( ppohBEM_face2node(1, 1), &
				...
				comm, ierr )
			if( ierr .ne. 0 )   print *, 'Error: MPI_Bcast#13'

			if( ppohBEM_nint_para_fc .gt. 0 ) then
				allocate( ppohBEM_int_para_fc(ppohBEM_nint_para_fc, ppohBEM_nofc), stat = ierr)
				if( ierr .ne. 0 ) then
			...
			if( ppohBEM_ndble_para_fc .gt. 0 ) then
				allocate( ppohBEM_dble_para_fc(ppohBEM_ndble_para_fc, ppohBEM_nofc), stat = ierr )
				if( ierr .ne. 0 )     print *, 'Memory allocation #13 failed'

				do i = 1, ppohBEM_nofc
					read( iunit, * ) ( ppohBEM_dble_para_fc(j, i), j = 1, ppohBEM_ndble_para_fc)
				enddo
				call MPI_Bcast ( ppohBEM_dble_para_fc(1, 1), &
					ppohBEM_nofc * ppohBEM_ndble_para_fc, &
					MPI_DOUBLE_PRECISION, 0, comm, ierr )
				if( ierr .ne. 0 )     print *, 'Error: MPI_Bcast#15 !!!'
			endif
			close ( iunit )
		else
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!***Print_result
	subroutine Print_result( filename, ppohBEM_nond, ppohBEM_nofc, &
		...
		do i = 1, ppohBEM_nond
			write( iunit, '(f16.13, 1x, f16.13, 1x, f16.13)' ) &
				st_ppohBEM_np(i)%x, st_ppohBEM_np(i)%y, st_ppohBEM_np(i)%z
		enddo
		write( iunit, '(a, i0, 1x, i0)' ) "CELLS ", ppohBEM_nofc, (ppohBEM_nond_on_face+1) * ppohBEM_nofc
		do i = 1, ppohBEM_nofc
...
!$omp end master
		...
		if( rnorm .lt. ppohBEM_tor*bnorm )   exit   !!! end iteration
