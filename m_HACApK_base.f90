...
!   integer, allocatable :: productedFlag(:)
...
!*** type :: st_HACApK_leafmtx_32
  type :: st_HACApK_leafmtx_32
    ...
    integer*4 nstrtt, ndt;
!   integer, allocatable :: productedFlag(:)
    real*4, pointer :: a1(:, :) => null(), a2(:, :) => null()
  end type st_HACApK_leafmtx_32

!*** type :: st_HACApK_leafmtxp_32
  type :: st_HACApK_leafmtxp_32
    integer*4 nd ! number of unknowns
    integer*4 nlf ! number of sub-matrices
    integer*4 nlfkt ! number of low-rank sub matrices
    integer*4 ktmax
    type(st_HACApK_leafmtx_32), pointer :: st_lf(:)=>null()
  end type st_HACApK_leafmtxp_32

!*** Scaling Experiment =========================================================

  type :: st_HACApK_leafmtx_scaled
    integer*4 ltmtx  ! kind of the matrix; 1:rk 2:full
    integer*4 kt
    integer*4 nstrtl, ndl;
    integer*4 nstrtt, ndt;
!   integer, allocatable :: productedFlag(:)
    real*8, pointer :: a1(:, :) => null(), a2(:, :) => null(), diag(:) => null()
  end type st_HACApK_leafmtx_scaled

  type :: st_HACApK_leafmtxp_scaled
    integer*4 nd ! number of unknowns
    integer*4 nlf ! number of sub-matrices
    integer*4 nlfkt ! number of low-rank sub matrices
    integer*4 ktmax
    type(st_HACApK_leafmtx_scaled), pointer :: st_lf(:)=>null()
  end type st_HACApK_leafmtxp_scaled

!*** Scaled 32 Experiment =========================================================

  type :: st_HACApK_leafmtx_scaled_32
    integer*4 ltmtx  ! kind of the matrix; 1:rk 2:full
    integer*4 kt
    integer*4 nstrtl, ndl;
    integer*4 nstrtt, ndt;
!   integer, allocatable :: productedFlag(:)
    real*4, pointer :: a1(:, :) => null(), a2(:, :) => null(), diag(:) => null()
  end type st_HACApK_leafmtx_scaled_32

  type :: st_HACApK_leafmtxp_scaled_32
    integer*4 nd ! number of unknowns
    integer*4 nlf ! number of sub-matrices
    integer*4 nlfkt ! number of low-rank sub matrices
    integer*4 ktmax
    type(st_HACApK_leafmtx_scaled_32), pointer :: st_lf(:)=>null()
  end type st_HACApK_leafmtxp_scaled_32

  !*** Mixed Experiment =========================================================

  type :: st_HACApK_leafmtx_mixed
    integer*4 ltmtx  ! kind of the matrix; 1:rk 2:full
    integer*4 kt, kt_64, kt_32
    integer*4 nstrtl, ndl;
    integer*4 nstrtt, ndt;
!   integer, allocatable :: productedFlag(:)
    real*8, pointer :: a1_64(:, :) => null(), a2_64(:, :) => null(), diag(:) => null()  ! diag is full 64
    real*4, pointer :: a1_32(:, :) => null(), a2_32(:, :) => null()
  end type st_HACApK_leafmtx_mixed

  type :: st_HACApK_leafmtxp_mixed
    integer*4 nd ! number of unknowns
    integer*4 nlf ! number of sub-matrices
    integer*4 nlfkt ! number of low-rank sub matrices
    integer*4 ktmax
    type(st_HACApK_leafmtx_mixed), pointer :: st_lf(:)=>null()
  end type st_HACApK_leafmtxp_mixed
...
!!!!!!!!!!!!!!!!!! start construction of H-matrix  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do ip = 1, nlf
      ...
      if(il<ilp) then
        print *, 'Error!; HACApK_generate_frame_leafmtx row_sort';
      elseif(il>ilp) then
...
!***HACApK_setcutthread
  subroutine HACApK_setcutthread(lthr, st_leafmtxp, st_ctl, mem8, nthr, ktp)
    ...
    do il = 1, nlf
      ...
      ndl   = st_leafmtxp%st_lf(il)%ndl
      ndt   = st_leafmtxp%st_lf(il)%ndt
      if(ltmtx==1) then
...
!***HACApK_count_lntmx
  RECURSIVE subroutine HACApK_count_lntmx(st_cltl, st_cltt, param, lpmd, lnmtx, nofc, nffc)
    ...
    zdistlt = dsqrt(zs)
    zeta = param(51)
    if( (st_cltl%zwdth <= zeta * zdistlt .or. st_cltt%zwdth <= zeta * zdistlt) .and. &
      (ndl >= nleaf .and. ndt >= nleaf .and. ndl <= nlmax .and. ndt <= nlmax) ) then
        lnmtx(1) = lnmtx(1) + 1
    else
