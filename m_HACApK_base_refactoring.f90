...
!!!!!!!!!!!!!!!!!! end clustering !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		ndpth = 0
		call constructionOfHmatrix(ndpth, lnmtx, st_clt, param, lpmd, nofc, nffc, st_ctl, mpinr, st_leafmtxp, nlf, st_leafmtx, ilp, ips)
...
!!!!!!!!!!!!!!!!!! start MPI load balance  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		lpmd(6) = lhp(mpinr + 1) + 1
		lpmd(7) = lhp(mpinr + 2)
		lpmd(5) = lpmd(7)-lpmd(6) + 1
		call MPI_Barrier( icomm, ierr )
...
!  write(*, 1000) 'irank = ', mpinr, ' ndlf_s = ', lpmd(11), ', ndlf_e = ', lpmd(12)
		...
		do ip = lpmd(11), lpmd(12)
			ltmtx = st_leafmtx(ip)%ltmtx
			ndl = st_leafmtx(ip)%ndl
			ndt = st_leafmtx(ip)%ndt
			ns = ndl * ndt
			if(ltmtx==1) then
				lnmtx(1) = lnmtx(1) + 1
				mem8 = mem8 + (ndt + ndl) * ktp
			else
				lnmtx(2) = lnmtx(2) + 1
				mem8 = mem8 + ns
			endif
...
!***constructionOfHmatrix
	subroutine constructionOfHmatrix(lnmtx, st_clt, param, lpmd, nofc, nffc, st_ctl, mpinr, st_leafmtxp, nlf, st_leafmtx, ilp, ips)
		integer*4 :: nofc, nffc, mpinr, nlf, ilp, ips
		type(st_HACApK_cluster) :: st_clt
		integer*4 :: lnmtx(3)
		real*8, pointer :: param(:)
		...
		do il = 1, nofc
			...
			enddo
		enddo
	endsubroutine constructionOfHmatrix
...
! zdistlt = max(dsqrt(zs)-st_cltl%zwdth/ndl-st_cltt%zwdth/ndt, 0.0)
		...
		elseif(nnsonl==0 .or. nnsont==0 .or. ndl <= nleaf .or. ndt <= nleaf) then
			nlf = nlf + 1
			st_leafmtx(nlf)%nstrtl = nstrtl; st_leafmtx(nlf)%ndl = ndl
			st_leafmtx(nlf)%nstrtt = nstrtt; st_leafmtx(nlf)%ndt = ndt
...
!     print *, 'ncut = ', ncut, '; zlmid = ', zlmid
			...
			nr = nd
			do while(nl < nr)
				do while(nl < nd .and. zgmid(lod(nl), ncut) <= zlmid)
					nl = nl + 1
				enddo
				do while(nr >= 0 .and. zgmid(lod(nr), ncut) > zlmid)
					nr = nr-1
				enddo
				if(nl < nr) then
					nh = lod(nl)
