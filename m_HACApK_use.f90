...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!           called in  m_ppohBEM_bembb2hacapk   bembb2hacapk        !!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!*** HACApK_gensolv
...
!*** HACApK_generate
	integer function HACApK_generate(st_leafmtxp, st_bemv, st_ctl, coord, ztol)
		...
		!---------------------------------------------------------
		!------- All subrioutines are in m_HACApK_base -----------
		!---------------------------------------------------------
		...
		if(st_ctl%param(1)>0 .and. mpinr==0) then
			write(*, 1000) 'nd = ', nd, ' nofc = ', nofc, ' nffc = ', nffc
			write(*, 1000) 'nrank = ', nrank, ' nth = ', nthr
		endif
		if(st_ctl%param(1)>1 .and. mpinr==0) then
			print *, 'param:'
			write(*, 10000) st_ctl%param(1:100)
		endif
		...
		elseif(st_ctl%param(61)==3) then
			ndnr_s = st_ctl%lpmd(6)
			ndnr_e = st_ctl%lpmd(7)
			ndnr = st_ctl%lpmd(5)
			
			allocate(st_bemv%ao(nd))
			st_bemv%ao(:) = 0.0d0
		...
		lnps = nd+1
		lnpe = 0
		if(st_leafmtxp%nlf<1)		print *, 'ERROR!; sub HACApK_generate; irank = ', mpinr, ' nlf = ', st_leafmtxp%nlf
...
! call HACApK_fill_leafmtx(st_leafmtxp%st_lf, st_bemv, st_ctl%param, znrm, st_ctl%lpmd, lnmtx, st_ctl%lod, st_ctl%lod, nd, st_leafmtxp%nlf, lnps, lnpe)
		...
		if(st_ctl%param(1)>0) then
			write(mpilog, 2000)  'time_supermatrix             =', st_create_hmtx - st_s
			write(mpilog, 2000)  'time_fill_hmtx               =', st_fill_hmtx - st_cal_matnorm
			write(mpilog, 2000)  'time_construction_Hmatrix    =', st_fill_hmtx - st_s
		endif
		
		if(st_ctl%param(1)>0 .and. mpinr==0) then
			print *, 'time_supermatrix             =', st_create_hmtx - st_s
			print *, 'time_fill_hmtx               =', st_fill_hmtx - st_cal_matnorm
			print *, 'time_construction_Hmatrix    =', st_fill_hmtx - st_s
		endif
...
!*** HACApK_solve
	integer function HACApK_solve(st_leafmtxp, st_bemv, st_ctl, rhs, sol, ztol)
		...
		integer*4, pointer :: lpmd(:), lthr(:), lod(:)
		1000 format(5(a, i10)/)
		2000 format(5(a, 1pe15.8)/)
...
!   do il = ndnr_s, ndnr_e
		...
		if(param(83)>0) then			! param(83) is mstep
			allocate(ao(nd), www(nd))
...
!$omp parallel
!		print *, 'myrank ', omp_get_thread_num()
!$omp end parallel
			
			print *, 'param(85) is ', param(85)
