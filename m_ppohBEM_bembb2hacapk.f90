...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!           called in  bem-bb-fw-HACApK-0.4.1   lrtrn        !!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!*** bembb2hacapk
...
! nint_para_fc      ; number of integer parameters
	integer function bembb2hacapk(st_bemv, st_ctl, np, face2node, dble_para_fc, rhs, sol, ztol, &
		...
		st_bemv%ndble_para_fc = ndble_para_fc
		st_bemv%nond_on_face = nond_on_face
		st_bemv%number_element_dof = number_element_dof
		st_bemv%face2node = face2node
		...
		do il=1, nond
			st_bemv%zx(il) = np(il)%x
			st_bemv%zy(il) = np(il)%y
			st_bemv%zz(il) = np(il)%z
		enddo
